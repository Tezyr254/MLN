...
export function Timeline({ onScrollToHero }: TimelineProps) {
  ...
  const progress = Math.abs(offset) / Math.max(1, trackLength * scale - containerSize);
  ...
  const handleSelectEvent = (event: TimelineEvent, index: number) => {
    focusEvent(event, index);
  };
  ...
  return (
    ...
    >
      ...
      <div className="relative px-6 pb-8 pt-24 md:px-12 md:pt-32">
        ...
        {!isMobile && <TimelineProgress progress={progress} isVertical={false} />}
        ...
        >
          {isMobile ? (
            <div className="relative flex flex-col py-8">
              <div
                className="absolute left-6 top-8 bottom-8 w-px bg-gradient-to-b from-mln-gold/20 via-mln-gold/60 to-mln-gold/20 md:left-8"
                aria-hidden="true"
              />
              {timelineEvents.map((event, index) => (
                ...
                >
                  <TimelineNode
                    ...
                    isVertical
                    onSelect={(e) => handleSelectEvent(e, index)}
                    index={index}
                  />
                </div>
              ))}
            ...
            <motion.div
              ...
              animate={
                isDragging ? undefined : { transition: { type: 'spring', stiffness: 300, damping: 35 } }
              }
            >
              <div
              ...
              <div
                ...
                style={{ width: trackLength, height: 0 }}
              >
                {timelineEvents.map((event, index) => {
              ...
              <div
                className="absolute bottom-6 flex justify-between text-xs text-mln-white/30"
